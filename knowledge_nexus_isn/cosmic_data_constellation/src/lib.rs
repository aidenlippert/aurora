...
fn find_node_id_by_did_property(did_string_to_find: &str, nodes_db: &HashMap<String, IsnNode>) -> Option<String> {
    // println!("[ISN_CDC_Debug] Searching for DID: {}", did_string_to_find);
    for (node_id_key, node_value) in nodes_db.iter() {
        // println!("[ISN_CDC_Debug] Checking Node ID: {}, Type: {}, Props: {:?}", node_id_key, node_value.r#type, node_value.properties);
        if node_value.r#type == "CelestialIdentity" {
            if let Some(prop_did) = node_value.properties.get("did") {
                // println!("[ISN_CDC_Debug]   Found DID property: {}", prop_did);
                if prop_did == did_string_to_find {
                    // println!("[ISN_CDC_Debug]   MATCH FOUND for Node ID: {}", node_id_key);
                    return Some(node_id_key.clone());
                }
            }
        }
    }
    // println!("[ISN_CDC_Debug] No match found for DID: {}", did_string_to_find);
    None
}
...
fn create_and_store_isn_node( /* ... same ... */
    ...
    // Ensure the main subject key-value is present, even if it was already in properties
...
pub fn record_identity_creation(
    did: &str, blk_h: u64, mut details: HashMap<String,String>, // `mut` to allow inserting "did"
) -> Result<IsnNode,String> {
    // Ensure "did" is in the properties map before calling create_and_store_isn_node,
    // which uses "did" as main_subject_key.
    details.insert("did".to_string(), did.to_string());
    create_and_store_isn_node("identity","CelestialIdentity",blk_h,details,"did",did)
}
// ... (rest of record_... functions are the same) ...
...
pub fn create_isn_edge( /* ... same as before, uses find_node_id_by_did_property ... */
...
pub fn get_isn_node(node_id: &str) -> Option<IsnNode> { /* ... same ... */
    println!("[ISN_CDC] Attempting to get node {} (mock)", node_id);
    ISN_MOCK_DB_NODES.lock().unwrap().get(node_id).cloned()
}
pub fn get_edges_from_node(node_id_or_did: &str, relationship_filter: Option<&str>) -> Vec<IsnEdge> { /* ... same, uses find_node_id_by_did_property ... */
    let nodes_db_guard = ISN_MOCK_DB_NODES.lock().unwrap();
    let actual_node_id = if nodes_db_guard.contains_key(node_id_or_did) { node_id_or_did.to_string() }
                       else { find_node_id_by_did_property(node_id_or_did, &nodes_db_guard).unwrap_or_else(|| node_id_or_did.to_string()) };
    drop(nodes_db_guard);
    let edges_db = ISN_MOCK_DB_EDGES.lock().unwrap();
    edges_db.iter().filter(|edge| edge.from_node_id == actual_node_id || edge.to_node_id == actual_node_id).filter(|edge| relationship_filter.map_or(true, |filter| edge.relationship_type == filter)).cloned().collect()
}
