...
EOF# Make sure you are in the aurora root directory: cd ~/aurora

# Add new dependencies to simulation_runner's Cargo.toml
if ! grep -q 'soulstar_matrix_identity =' tests/simulation_runner/Cargo.toml; then
    sed -i '/^\[dependencies\]/a soulstar_matrix_identity = { path = "../../astral_hyper_engines/cluster_alpha_economic_sovereign/soulstar_matrix_identity" }' tests/simulation_runner/Cargo.toml
fi
if ! grep -q 'symbiotic_trust_lattice_stl =' tests/simulation_runner/Cargo.toml; then
    sed -i '/^\[dependencies\]/a symbiotic_trust_lattice_stl = { path = "../../trust_obligation_stl_von/symbiotic_trust_lattice_stl" }' tests/simulation_runner/Cargo.toml
fi
if ! grep -q 'verifiable_obligation_nexus_von =' tests/simulation_runner/Cargo.toml; then
    sed -i '/^\[dependencies\]/a verifiable_obligation_nexus_von = { path = "../../trust_obligation_stl_von/verifiable_obligation_nexus_von" }' tests/simulation_runner/Cargo.toml
fi


cat << 'EOF' > tests/simulation_runner/src/main.rs
use aethercore_runtime::ExecutionRequest;
use ecliptic_concordance::{Transaction as ConsensusTransaction, Block};
